...
/*
 * Reset entire system, XEBRA board rev3
 */
static int xebra_panel_reset_device(struct i2c_client *client,
				    unsigned long code)
{
	u8 addr_buf[3];
	struct i2c_msg msgs[1];
	...
	if (code == SYS_POWER_OFF || code == SYS_HALT)
		addr_buf[0] = SHUTDOWN_DEVICE;
	else
		addr_buf[0] = RESET_DEVICE;
...
static int xebra_panel_reboot_event(struct notifier_block *this,
				    unsigned long code, void *ptr)
{
	struct xebra_panel_regulator *state =
		container_of(this, struct xebra_panel_regulator,
			     reboot_notifier);

	(void) xebra_panel_reset_device(state->i2c, code);
	return NOTIFY_DONE;
}
...
static int xebra_panel_i2c_probe(struct i2c_client *i2c)
{
	...
	state->reboot_notifier.notifier_call = xebra_panel_reboot_event;
	register_reboot_notifier(&state->reboot_notifier);
...
static void xebra_panel_i2c_remove(struct i2c_client *client)
{
	...
	unregister_reboot_notifier(&state->reboot_notifier);
	mutex_destroy(&state->lock);
}
