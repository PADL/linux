			   ...
			   u16 *cont_code)
{
	...
	if (ret < resp_len || (ret - resp_len) % sizeof(struct mv88e6xxx_rmu_atu_entry)) {
		dev_err(chip->dev, "RMU: DUMP_ATU returned wrong length %zd %d\n",
			sizeof(resp), ret);
		return -EPROTO;
	}
				     ...
				     const struct mv88e6xxx_atu_entry *entry)
{
	...
	if (ret < resp_len) {
		dev_err(chip->dev, "RMU: write MAC DATA returned wrong length %d %d\n",
			resp_len, ret);
		return -EPROTO;
	}
				    ...
				    struct mv88e6xxx_atu_entry *entry)
{
	...
	/* this appears to be required to avoid previous MAC address being read back */
	...
	if (ret < resp_len) {
		dev_err(chip->dev, "RMU: read MAC DATA returned wrong length %d %d\n",
			resp_len, ret);
		return -EPROTO;
	}
