...
/* AVTP has been assigned the entire 90:E0:F0 range (IEEE 1722 Annex D) */
static const u8 eth_avtp_mcast_addr_base[ETH_ALEN] __aligned(2) =
{ 0x91, 0xe0, 0xf0, 0x00, 0x00, 0x00 };

static inline bool ether_addr_is_avtp_mcast(const u8 *addr)
{
	u8 mask[ETH_ALEN] = { 0xff, 0xff, 0xff, 0x80, 0x00, 0x00 };

	return ether_addr_equal_masked(addr, eth_avtp_mcast_addr_base, mask);
}
