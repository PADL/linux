...
static int mv88e6xxx_setup(struct dsa_switch *ds)
{
	...
	if (chip->rmu_state == MV88E6XXX_RMU_ONLY_ENABLED) {
		WARN_ON(chip->rmu_conduit != NULL);
		WARN_ON(!ds->inband_only);
...
static const struct mv88e6xxx_qav_info mv88e6352_qav_info = {
	...
	.avb_pri_map = 0x5342,
	/* non-AVB HI FPri 3/QPri 1, LO FPri 2/QPri 1  */
	.avb_legacy_pri_map = 0x3121,
};
...
static const struct mv88e6xxx_qav_info mv88e6390_qav_info = {
	...
	.avb_pri_map = 0x3726,
	/* non-AVB HI FPri 1/QPri 3, LO FPri 1/QPri 2  */
	.avb_legacy_pri_map = 0x1312,
};
				  ...
				  struct dsa_db db)
{
	 ...
	 */
	if ((mdb->flags & SWITCHDEV_MDB_F_DYNAMIC_RESERVATION) && chip->info->qav)
	...
	if (mv88e6xxx_atu_mc_entry_changed(&existing, port, mdb, state)) {
		dev_info_ratelimited(chip->dev,
				     "p%d: cannot offload MDB %pM: dynamic reservation state conflicts with existing entry\n",
				     port, mdb->addr);
		err = -EINVAL;
		goto out;
	}
	...
	if (pdata) {
		...
		for (port = 0; port < DSA_MAX_PORTS; port++) {
			if (!(pdata->enabled_ports & (1 << port)))
				continue;
			if (!pdata->cd.port_names[port] ||
			    strcmp(pdata->cd.port_names[port], "cpu"))
				continue;
