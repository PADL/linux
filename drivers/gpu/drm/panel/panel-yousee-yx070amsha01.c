...
static int yx070amsha01_update_status(struct backlight_device *bl)
{
	...
	/* brightness on the hardware is 1-4096 */
	ret = mipi_dsi_dcs_set_display_brightness_large(dsi, brightness + 1);
	if (ret < 0)
		return ret;
...
yx070amsha01_create_backlight(struct mipi_dsi_device *dsi)
{
	struct device *dev = &dsi->dev;
	const struct backlight_properties props = {
		.type = BACKLIGHT_RAW,
		.brightness = YX070AMSHA01_DEFAULT_BRIGHTNESS,
		.max_brightness = YX070AMSHA01_MAX_BRIGHTNESS,
	};

	return devm_backlight_device_register(dev, dev_name(dev), dev, dsi,
					      &yx070amsha01_bl_ops, &props);
}
...
static int yx070amsha01_init_sequence(struct drm_panel *panel)
{
	...
	/* DCS: Set display brightness control and backlight */
	...
	mipi_dsi_generic_write_seq_multi(
		&dsi_ctx, MIPI_DCS_SET_DISPLAY_BRIGHTNESS, 0x00, 0x01);
...
yx070amsha01_panel_get_orientation(struct drm_panel *panel)
{
	struct yx070amsha01_panel *ctx = panel_to_yx070amsha01_panel(panel);
...
static const struct drm_panel_funcs yx070amsha01_panel_funcs = {
	...
	.get_orientation = yx070amsha01_panel_get_orientation,
...
static int yx070amsha01_panel_dsi_probe(struct mipi_dsi_device *dsi)
{
	...
	if (ret < 0)
		return dev_err_probe(&dsi->dev, ret, "Invalid rotation\n");

	ctx->panel.prepare_prev_first = true;
	drm_panel_init(&ctx->panel, &dsi->dev, &yx070amsha01_panel_funcs,
		       DRM_MODE_CONNECTOR_DSI);

	ctx->panel.backlight = yx070amsha01_create_backlight(dsi);
